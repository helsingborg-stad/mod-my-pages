...
class LoginUrlServiceFactory
{
    ...
    {
        $apiUrl = fn(): string => Settings::apiUrl();
        $mockApiUrl = fn(): string => !empty($mockUrl) ? $mockUrl : 'http://localhost:3000';

        return defined('PHPUNIT_RUNNING') ? $mockApiUrl : $apiUrl;
    }

    private static function createHomeUrl(string $mockUrl = ''): \Closure
    {
        $homeUrl = fn(): string => home_url();
        $mockHomeUrl = fn(): string => !empty($mockUrl) ? $mockUrl : 'http://example.test';

        return defined('PHPUNIT_RUNNING') ? $mockHomeUrl : $homeUrl;
    }
    ...
    private static function createDefaultCallbackUrl(string $mockUrl = ''): \Closure
    {
        ...
        $mockDefaultCallbackUrl = fn(): string => !empty($mockUrl)
            ? $mockUrl
            : 'http://example.test/mina-sidor';

        return defined('PHPUNIT_RUNNING') ? $mockDefaultCallbackUrl : $defaultCallbackUrl;
    }
